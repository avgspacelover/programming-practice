...
A is coded as ·−, letter Q is coded as −−·−, and digit 1 is coded as ·−−−−. 
...
decodeMorse = function(morseCode){
    ...
    let wordStr = morseCode.split("   ")
    console.log("wordSTR",wordStr)
    wordStr=wordStr.filter((item,index)=>{
      if(index==0){
        if(item!==" "){
          return item;
        }
    ...
    wordStr.map((item,index)=> {
      ...
      let letterStr= item.split(" ")
      letterStr.map((element,i)=>{
        text=[]
        text.push(MORSE_CODE[element])
